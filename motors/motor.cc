...
void Motor::HandleInterrupt() {
  ...
  {
    CloseAdcReading close1, close2;

    // 98 - 898
    const int ticks_per_sample = 100;
    int sample_point = 133;
    for (int reading = 0; reading < 2; ++reading) {
      for (int phase = 0; phase < 3; ++phase) {
#if 0
        const int distance = MinimumDistanceTo(sample_point);
        if (distance == INT_MAX) {
          temp2 = 0xFFF;
        }
#if 1
        CloseAdcReading *const closer =
            (close1.distance < close2.distance) ? &close1 : &close2;
        CloseAdcReading *const farther =
            (close1.distance < close2.distance) ? &close2 : &close1;
#else
        CloseAdcReading *const closer = &close1;
        CloseAdcReading *const farther = &close2;
#endif
        if (!farther->MaybeUse(distance, adc_readings, phase, reading,
                               &to_balance)) {
          if (!closer->MaybeUse(distance, adc_readings, phase, reading,
                                &to_balance)) {
            temp1 |= 1 << (reading * 3 + phase);
            to_balance.Add(phase, adc_readings.motor_currents[phase][reading]);
          } else {
            temp2 |= 1 << (reading * 3 + phase);
          }
        } else {
          temp1 |= 1 << (reading * 3 + phase + 8);
        }
#else
        to_balance.Add(phase, adc_readings.motor_currents[phase][reading]);
#endif
        sample_point += ticks_per_sample;
      }
      sample_point += ticks_per_sample;
    }
  }
  ...
  if (!high_gear) {
    current_now = current_now * -135.0f / 120.0f;
  } else {
    current_now = current_now * 130.0f / 120.0f;
  }
...
#if 0
  const float foo = (current_now > 1.0f || current_now < -1.0f) ? 100.0f : 0;
#endif
  ...
  pwm_ftm_->C5V = 0;
#elif 0
  int position = static_cast<int32_t>(encoder_ftm_->CNT) - 75;
  if (position < 0) {
    position += 1024;
  }
...
// 1 5 3: works?
#if 0
  const uint32_t kOn = 80;
#else
  const uint32_t kOn = 0;
#endif
  if (position < 171) {
    pwm_ftm_->C1V = kOn;
    ...
    pwm_ftm_->C3V = kOn;
  } else if (position < 683) {
    pwm_ftm_->C3V = kOn;
    pwm_ftm_->C5V = kOn;
  } else if (position < 853) {
    pwm_ftm_->C3V = kOn;
    pwm_ftm_->C1V = kOn;
  } else {
    pwm_ftm_->C1V = kOn;
...
int Motor::MinimumDistanceTo(int sample_point) const {
  int minimum = INT_MAX;
#if 0
  for (uint32_t edge :
       ::std::array<uint32_t, 3>{pwm_ftm_->C1V, pwm_ftm_->C3V, pwm_ftm_->C5V}) {
#elif 1
  for (uint32_t edge :
       ::std::array<uint32_t, 6>{pwm_ftm_->C0V, pwm_ftm_->C1V, pwm_ftm_->C2V,
                                 pwm_ftm_->C3V, pwm_ftm_->C4V, pwm_ftm_->C5V}) {
#elif 1
  for (uint32_t edge : ::std::array<uint32_t, 1>{pwm_ftm_->C3V}) {
#endif
    int distance = static_cast<int>(edge) - sample_point;
    if (distance < 0) {
      distance = -distance;
    }
    if (distance < minimum) {
