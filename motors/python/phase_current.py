...
R_matrix = numpy.matrix(numpy.eye(3)) * R
L_matrix = numpy.matrix([[L, M, M], [M, L, M], [M, M, L]])

L_matrix_inv = numpy.linalg.inv(L_matrix)
...
print 'Max drive voltage', max(one_amp_driving_voltage)
...
class Simulation(object):
  def __init__(self):
    self.A = L_matrix_inv * R_matrix
    self.B = L_matrix_inv * Vconv
    ...
    self.X = numpy.matrix([[0.0], [0.0], [0.0], [0.0], [1200.0], [0.0], [0.0], [0.0]])
  ...
  def motor_diffeq(self, x, t, U):
    ...
    di_dt = -self.A * I + self.B * U - L_matrix_inv * dflux * omega
    torque = I.T * dflux
    Xdot[0:3, :] = di_dt
    Xdot[3, :] = omega
    Xdot[4, :] = torque / J
  ...
  def DoControls(self, goal_current):
    ...
    #print 's'
    #print (E_imag1 + E_imag2).real.T * phases(g_single, theta)
    #print phases(f_single, theta).T * measured_current

    H1 = -numpy.linalg.inv(1j * omega * L_matrix + R_matrix) * omega / Kv
    H2 = -numpy.linalg.inv(1j * omega * 5.0 * L_matrix + R_matrix) * omega / Kv
    ...
    #p_next = A_discrete * (X[0:3, :] - p) + B_discrete * Vn + p_next_imag.real
    ...
    self.data_logger.log_controls(self.current_time, measured_current, Icurrent, E)
  ...
  def Simulate(self):
    ...
    for n in range(200):
      goal_current = 10.0
      max_current = (vcc - abs(self.X[4, 0] / Kv * 2.0)) / max_one_amp_driving_voltage
      goal_current = min(max_current, goal_current)
